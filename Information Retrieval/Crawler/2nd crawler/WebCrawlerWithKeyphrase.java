...
public class WebCrawlerWithKeyphrase {
	 ...
	 * @throws IOException
	 */
	public void get(String url, String keyphrase) throws IOException {
		// set the connection
		...
		String result = formatter.getPlainText(doc).replace("\n", "");
		// check if the page contains keyphrase
		if (result.toLowerCase().contains(keyphrase)) {
			...
			for (Element e : hrefs) {
				...
				// not follow links with a colon (:) in the rest of the URL
						...
						// not follow links already crawled and only follow
						// links at most depth 3
					...
					Elements redirectUrl = document
							.select("link[rel=canonical]");
					/*
					 * check if the canonical link is different from original
					 * link and if the canonical link is already in the hashmap
					 */
					if (!redirectUrl.attr("abs:href")
							.equals(e.attr("abs:href"))
	 ...
	 */
	public void crawl(String seedUrl, String keyphrase) throws IOException,
			InterruptedException {
		q.add(seedUrl);
		m.put(seedUrl, 1);
		while (!q.isEmpty()) {
			get(q.remove(), keyphrase);
